...
const ProductCard = ({ product }) => {
  ...
  // --- FIX: Get the first available image from the new structure ---
  const getThumbnail = () => {
    // Check if colors and imagesByColor exist
    if (product.colors && product.colors.length > 0 && product.imagesByColor) {
      // Get the first color
      const firstColor = product.colors[0]
      // Get the images for that color
      const imagesForFirstColor = product.imagesByColor[firstColor]
      // Return the first image of that color, if it exists
  ...
  return (
    ...
    >
      <div className='cursor-pointer group relative bg-gray-500/10 rounded-lg w-full h-52 flex items-center justify-center'>
        <Image
          src={thumbnailUrl} // Use the new thumbnail URL
          alt={product.name}
